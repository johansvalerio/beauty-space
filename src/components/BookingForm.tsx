...
export default function BookingForm(session: BookingFormProps) {
    ...
    async function handleSubmit(e: FormEvent<HTMLFormElement>) {
        ...
        console.log(service, speciality, date, session.session?.user?.id);
    ...
    return (
        <section id="contact" className="scroll-mt-28 m-4 md:m-16">
            ...
            <article className="grid grid-cols-1 md:grid-cols-1 md:justify-items-center gap-8 relative">
                ...
                {
                    !session.session && <div className="absolute rounded-lg inset-0  bg-rose-300/60 bg-opacity-20 flex items-center justify-center">
                        <Link href="/auth/signin" className="text-xl bg-gray-700 rounded hover:bg-gray-800 text-white p-4">Inicia sesión para agendar tu cita</Link>
                    </div>
                }
            </article>
        </section>
